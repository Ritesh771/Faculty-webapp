...
import { getProctorStudents, manageStudentLeave, ProctorStudent } from '@/utils/faculty_api';
...
const ManageStudentLeavePage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.success && res.data) {
        const rows: LeaveApplication[] = [];
        res.data.forEach((s: ProctorStudent) => {
          (s.leave_requests || []).forEach(l => {
            rows.push({
              id: l.id,
              studentName: s.name,
              usn: s.usn,
  ...
  const handleApprove = async (application: LeaveApplication) => {
    const res = await manageStudentLeave({ leave_id: application.id, action: 'APPROVE' });
    if (res?.success) {
      setLeaveApplications(prev => prev.map(app => app.id === application.id ? { ...app, status: 'approved' } : app));
      toast({ title: 'Leave Approved', description: `Leave application for ${application.studentName} has been approved.` });
    } else {
      toast({ title: 'Error', description: res?.message || 'Failed to approve', variant: 'destructive' });
    }
  };
  ...
  const handleReject = async (application: LeaveApplication) => {
    ...
    const res = await manageStudentLeave({ leave_id: application.id, action: 'REJECT' });
    if (res?.success) {
      setLeaveApplications(prev => prev.map(app => app.id === application.id ? { ...app, status: 'rejected', rejectionReason } : app));
      toast({ title: 'Leave Rejected', description: `Leave application for ${application.studentName} has been rejected.`, variant: 'destructive' });
      setRejectionReason('');
      setIsRejectDialogOpen(false);
      setSelectedApplication(null);
    } else {
      toast({ title: 'Error', description: res?.message || 'Failed to reject', variant: 'destructive' });
    }
  };
