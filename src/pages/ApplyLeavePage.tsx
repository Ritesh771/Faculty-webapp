  ...
  useEffect(() => {
    (async () => {
      ...
      if (bootstrapRes?.success && bootstrapRes?.data) {
        const { assignments, leave_requests, branches: bootstrapBranches } = bootstrapRes.data;
        setBranches(bootstrapBranches || []);
        if (bootstrapBranches && bootstrapBranches.length > 0) setSelectedBranch(String(bootstrapBranches[0].id));
